...
class BaseWidget(GridLayout): #Is used in code
    pass
...
class MentorButton(Button): # Used to gererate dynamic buttons
    pass

#class MentorTextInput(TextInput):
#    pass
...
class BaseApp(App):
    ...
    def _write_cockpit(self, message, *args):
        if False:
            ...
            self.root.ids.the_exercises.text = text
    ...
    def build(self):
        ...
        for title in self.sequences.titles:
            btn = MentorButton(text=title)  #, size_hint_y=None, height=40)
            btn.bind(on_press=partial(self.sequences.get_osc_message_from_title, title, self.root.ids.rst_doc))
